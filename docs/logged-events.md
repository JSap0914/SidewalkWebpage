...
| `Click_RouteForwardArrow` / `RouteForwardArrow_Success` / `RouteForwardArrow_PanoNotAvailable` | Explore: the on-pano route-forward arrow (#4671), shown only when the imagery link graph offers no arrow along the route. Clicking it runs the same route-aware `moveForward()` as the compass's "straight" and the Stuck button, so its volume overlaps `CompassMove_*` / `ModalStuck_*` and `KeyboardShortcut_MoveForwardAlongRoute` (`usedRoute:true`) — but only at link-graph dead-ends, where the arrow appears. |
